    ...
    <div className="min-h-screen flex flex-col">
      ...
      {!isFreePurchase && (
        <header className="bg-gray-800 dark:bg-gray-900 text-white px-3 py-2 sm:px-4 sm:py-4 flex justify-between items-center flex-shrink-0 gap-2">
          <Link to={buildPathWithUTM(homePath)} className="flex items-center space-x-2 sm:space-x-3 font-bold min-w-0 flex-shrink-0">
          ...
          <div className="flex items-center gap-2 sm:gap-4 flex-shrink-0 min-w-0">
            <Link to={buildPathWithUTM(cartPath)} className="hover:underline text-xs sm:text-base whitespace-nowrap">
              Cart ({items.length})
            </Link>
